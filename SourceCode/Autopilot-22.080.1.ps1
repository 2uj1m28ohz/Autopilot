...
function CarregarNucleo {
	...
	$script:SoftwareVersao = "22.080.1"
	$script:SoftwareAlgoritmoVersao = "22.080.1"
	$script:SoftwareInterfaceVersao = "22.070.1"
	$script:SoftwareRequisitoSistemaOperacional = "14393"
...
function CarregarInterface {
	...
	$script:Suporte_RemoverEventosNavBit = "$ElementoNavBitOff"
	$script:Suporte_InstalarSoftwareNavBit = "$ElementoNavBitOff"
	$script:Suporte_AtualizarSoftwareNavBit = "$ElementoNavBitOff"
	...
	$script:Suporte_RemoverEventosTitulo = "MENU $ElementoSeparador INFORMAÇÕES $ElementoSeparador SUPORTE $ElementoSeparador REMOVER EVENTOS"
	...
	$script:Suporte_AtualizarSoftwareTitulo = "MENU $ElementoSeparador INFORMAÇÕES $ElementoSeparador SUPORTE $ElementoSeparador ATUALIZAR SOFTWARE"
	...
	$script:MenuDescricao = "$SoftwareNome é desenvolvido para executar rotinas de backup e manutenção do sistema"
	...
	$script:Suporte_RemoverEventosDescricao = "Remove todos os registros de eventos"
	...
	$script:AlertaSuporteFim = "O suporte à esta versão de $SoftwareNome foi encerrado"
...
function CarregarConfiguracoes {
	...
	$SoftwareInstalacaoStatus = 0
	$ItensLegadosStatus = 0
	$BitsTransferStatus = 0
	$EventosStatus = 1
	$script:SoftwareConfiguracoes = @{
		ArquivoConfiguracoesVersao = "$SoftwareVersao"
		SoftwareInstalacaoStatus = "$SoftwareInstalacaoStatus"
		ItensLegadosStatus = "$ItensLegadosStatus"
		BitsTransferStatus = "$BitsTransferStatus"
		EventosStatus = "$EventosStatus"
	}
	if ((Test-Path "$DiretorioSoftware\$ArquivoConfiguracoes") -eq $True) {
		...
		if ($SoftwareConfiguracoesTemporario.ArquivoConfiguracoesVersao -eq $SoftwareVersao) {
			$script:SoftwareConfiguracoes.SoftwareInstalacaoStatus = $SoftwareConfiguracoesTemporario.SoftwareInstalacaoStatus
			$script:SoftwareConfiguracoes.ItensLegadosStatus = $SoftwareConfiguracoesTemporario.ItensLegadosStatus
			$script:SoftwareConfiguracoes.BitsTransferStatus = $SoftwareConfiguracoesTemporario.BitsTransferStatus
			$script:SoftwareConfiguracoes.EventosStatus = $SoftwareConfiguracoesTemporario.EventosStatus
		} else {
			$script:SoftwareConfiguracoes.SoftwareInstalacaoStatus = $SoftwareConfiguracoesTemporario.SoftwareInstalacaoStatus
			$script:SoftwareConfiguracoes.ItensLegadosStatus = $SoftwareConfiguracoesTemporario.ItensLegadosStatus
			$script:SoftwareConfiguracoes.BitsTransferStatus = $SoftwareConfiguracoesTemporario.BitsTransferStatus
	...
	if ($SoftwareConfiguracoes.SoftwareInstalacaoStatus -eq $Null) {
		$script:SoftwareConfiguracoes.SoftwareInstalacaoStatus = 0
		$SoftwareConfiguracoes | ConvertTo-Json | Out-File -FilePath "$DiretorioSoftware\$ArquivoConfiguracoes" -Force
	}
	if (($SoftwareConfiguracoes.ItensLegadosStatus -eq 0) -or ($SoftwareConfiguracoes.ItensLegadosStatus -eq $Null)) {
		...
		$script:SoftwareConfiguracoes.ItensLegadosStatus = 1
		$SoftwareConfiguracoes | ConvertTo-Json | Out-File -FilePath "$DiretorioSoftware\$ArquivoConfiguracoes" -Force
	}
	if (($SoftwareConfiguracoes.BitsTransferStatus -eq 0) -or ($SoftwareConfiguracoes.BitsTransferStatus -eq $Null)) {
		Write-Host "$ElementoProgresso Instalando módulo BitsTransfer"
		Import-Module BitsTransfer -Force
		$script:SoftwareConfiguracoes.BitsTransferStatus = 1
		$SoftwareConfiguracoes | ConvertTo-Json | Out-File -FilePath "$DiretorioSoftware\$ArquivoConfiguracoes" -Force
	}
	if ($SoftwareConfiguracoes.EventosStatus -eq $Null) {
		$script:SoftwareConfiguracoes.EventosStatus = 1
		$SoftwareConfiguracoes | ConvertTo-Json | Out-File -FilePath "$DiretorioSoftware\$ArquivoConfiguracoes" -Force
	}
	if ($SoftwareConfiguracoes.EventosStatus -eq 1) {
		VerificarTempo
...
function VerificarAtualizacoes {
	...
	if ($SoftwareConfiguracoes.SoftwareInstalacaoStatus -eq 1) {
	...
	} elseif ($SoftwareConfiguracoes.SoftwareInstalacaoStatus -eq 0) {
		if ($SoftwareConfiguracoes.EventosStatus -eq 1) {
			VerificarTempo
...
function EveOnline_RestaurarBackup {
	...
	if (($OneDriveStatus -eq 1) -and ($EveOnline_RestaurarBackupOrigemStatus -eq 1) -and ($EveOnline_RestaurarBackupDestinoStatus -eq 1)) {
		...
		if ((Test-Path "$DiretorioTemporario\EVE Online\EVE") -eq $True) {
			...
			Start-Sleep 0.5
			Copy-Item "$DiretorioTemporario\EVE Online\EVE" -Destination "$DiretorioUsuario\Documents" -Recurse -Force
		} else {
			Write-Host "$ElementoErro $ErroDiretorio" -ForegroundColor "$CorErro"
		}
...
function Configuracoes_HabilitarEventos {
	...
	Write-Host "$ElementoDivisor" -ForegroundColor "$CorDivisor"
	$script:EventosStatus = 1
	if ((Test-Path "$DiretorioSoftware\$ArquivoConfiguracoes") -eq $True) {
		$script:SoftwareConfiguracoes.EventosStatus = $EventosStatus
		$SoftwareConfiguracoes | ConvertTo-Json | Out-File -FilePath "$DiretorioSoftware\$ArquivoConfiguracoes" -Force
...
function Suporte {
	...
	do {CarregarInterfaceCores
		...
		Write-Host "$Suporte_RemoverEventosNavBit" -ForegroundColor "$CorNavBit" -NoNewline; Write-Host " 2 " -ForegroundColor "$CorID" -NoNewline; Write-Host "Remover Eventos"
		Write-Host "$Suporte_InstalarSoftwareNavBit" -ForegroundColor "$CorNavBit" -NoNewline; Write-Host " 3 " -ForegroundColor "$CorID" -NoNewline; Write-Host "Instalar $SoftwareNome"
		Write-Host "$Suporte_DesinstalarSoftwareNavBit" -ForegroundColor "$CorNavBit" -NoNewline; Write-Host " 4 " -ForegroundColor "$CorID" -NoNewline; Write-Host "Atualizar $SoftwareNome"
		Write-Host "$Suporte_DesinstalarSoftwareNavBit" -ForegroundColor "$CorNavBit" -NoNewline; Write-Host " 5 " -ForegroundColor "$CorID" -NoNewline; Write-Host "Desinstalar $SoftwareNome"
		Write-Host "  X " -ForegroundColor "$CorID" -NoNewline; Write-Host "$SubRotinaRetornar"
		Write-Host ""
		Write-Host "$ElementoDivisor" -ForegroundColor "$CorDivisor"
		$Comando = Read-Host -Prompt "$ElementoInteracao $SubRotinaSelecao"
		switch ($Comando) {
			...
			2 {Suporte_RemoverEventos}
			3 {Suporte_InstalarSoftware}
			4 {Suporte_AtualizarSoftware}
			5 {Suporte_DesinstalarSoftware}
			X {}
			default {Erro}
		}
	} while ($Comando -ne "X")
...
function Suporte_RemoverEventos {
	if ($SoftwareConfiguracoes.EventosStatus -eq 1) {
		VerificarTempo
		Add-Content -Path "$DiretorioEventos\$ArquivoEventos" -Value "$ArquivoEventosLayout Suporte_RemoverEventos"
	}
	CarregarInterfaceCores
	Write-Host "$ElementoTitulo $Suporte_RemoverEventosTitulo" -BackgroundColor "$CorTitulo"
	Write-Host "$ElementoDescricao $Suporte_RemoverEventosDescricao" -BackgroundColor "$CorDescricao"
	Write-Host "$ElementoDivisor" -ForegroundColor "$CorDivisor"
	Remove-Item "$DiretorioEventos\*" -Recurse -Force | Out-Null
	Write-Host "$ElementoSucesso Eventos removidos" -ForegroundColor "$CorSucesso"
	Write-Host "$ElementoDivisor" -ForegroundColor "$CorDivisor"
	Read-Host -Prompt "$ElementoInteracao $SubRotinaSair"
	$script:Suporte_RemoverEventosNavBit = "$ElementoNavBitOn"
}
...
function Suporte_InstalarSoftware {
	...
	if ($SoftwareConfiguracoes.SoftwareInstalacaoStatus -eq 1) {
		...
		if ((Test-Path "$DiretorioTemporario\$SoftwareNome-$SoftwareVersao.7z") -eq $True) {
			...
			$script:SoftwareConfiguracoes.SoftwareInstalacaoStatus = 1
			$SoftwareConfiguracoes | ConvertTo-Json | Out-File -FilePath "$DiretorioSoftware\$ArquivoConfiguracoes" -Force
			Remove-Item "$DiretorioSoftware\*.ps1" -Recurse -Force | Out-Null
			Remove-Item "$DiretorioSoftware\*.html" -Recurse -Force | Out-Null
	...
	do {CarregarInterfaceCores
		...
		if ($SoftwareProblemas -gt 0) {
			if ($SoftwareSuporte -eq 0) {
				Write-Host "$ElementoAlerta $AlertaSuporteFim" -ForegroundColor "$CorAlerta"
			}
...
function Ajuda_PoliticaSuporte {
	...
	Write-Host "  Uma versão sem suporte não receberá mais atualizações de software. Estão incluídas atualizações"
	Write-Host "  de recursos, correções e otimizações que podem adicionar novas funcionalidades, corrigir problemas,"
	Write-Host "  e otimizar o desempenho e a confiabilidade geral do software."
	Write-Host ""
	Write-Host "  2. QUANDO TERMINA O SUPORTE À MINHA VERSÃO?" -ForegroundColor "$CorSecao"
	Write-Host "  As três versões recentes lançadas são cobertas pelo suporte. Se a versão instalada no seu dispositivo"
	Write-Host "  ainda for compatível, instale as atualizações disponíveis. Por exemplo, se você estiver executando a"
	Write-Host "  versão 22.100.1 e a versão 22.100.2 foi lançada, será necessário atualizar para a versão 22.100.2"
	Write-Host "  para se qualificar para o suporte."
	Write-Host ""
	...
	Write-Host "  Para receber atualizações basta instalar o software e estar conectado à internet. Ao iniciar, ele"
	Write-Host "  buscará e instalará atualizações automaticamente. Se preferir usá-lo sem instalar, a atualização"
	Write-Host "  deve ser feita manualmente."
	Write-Host "$ElementoDivisor" -ForegroundColor "$CorDivisor"
	Read-Host -Prompt "$ElementoInteracao $SubRotinaSair"
...
do {if ($SoftwareConfiguracoes.EventosStatus -eq 1) {
		VerificarTempo
	...
	Write-Host "$ElementoExpansor" -ForegroundColor "$CorExpansor" -NoNewline; Write-Host " A4 " -ForegroundColor "$CorID" -NoNewline; Write-Host "Drivers"
	Write-Host "$ExportarBackupsNavBit" -ForegroundColor "$CorNavBit" -NoNewline; Write-Host " A5 " -ForegroundColor "$CorID" -NoNewline; Write-Host "Exportar Backups"
	Write-Host ""
