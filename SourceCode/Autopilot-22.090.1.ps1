...
function IndustriesOfTitan {
	...
	do {CarregarInterfaceCores
		...
		Write-Host "$IndustriesOfTitan_RestaurarBackupNavBit" -ForegroundColor "$CorNavBit" -NoNewline; Write-Host " 2 " -ForegroundColor "$CorID" -NoNewline; Write-Host "Restaurar Backup" -ForegroundColor "DarkGray" -NoNewline; Write-Host " [Em breve]" -ForegroundColor "$CorDestaque"
		Write-Host "  X " -ForegroundColor "$CorID" -NoNewline; Write-Host "$SubRotinaRetornar"
		Write-Host ""
		Write-Host "$ElementoDivisor" -ForegroundColor "$CorDivisor"
		$Comando = Read-Host -Prompt "$ElementoInteracao $SubRotinaSelecao"
